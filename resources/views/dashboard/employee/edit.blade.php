<x-dashboard.main title="{{ __('employee.modal.edit_title', ['name' => $employee->user->name]) }}">
    <div class="max-w-6xl mx-auto">
        ...
        <form action="{{ route('employees.update', $employee->id) }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            @csrf
            @method('PUT')
            ...
            <div class="lg:col-span-4 space-y-6">
                <div class="bg-base-100/50 backdrop-blur-xl rounded-[2.5rem] p-8 border border-base-content/5 shadow-sm relative overflow-hidden h-full">
                    <div class="absolute -right-8 -top-8 w-32 h-32 bg-info/5 rounded-full blur-3xl"></div>
                    
                    <div class="flex items-center gap-4 mb-10">
                        <div class="size-14 rounded-2xl bg-info/10 flex items-center justify-center text-info shadow-inner">
                            <x-lucide-user-cog class="size-7" />
                        </div>
                        <div>
                            <h3 class="font-black text-xl tracking-tight">Account</h3>
                            <p class="text-[10px] font-bold opacity-30 uppercase tracking-[0.2em]">User Credentials</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-5">
                        <div class="form-control group">
                            ...
                            <label class="input input-bordered flex items-center gap-4 bg-base-200/50 border-base-content/5 rounded-2xl h-14 focus-within:border-info transition-all shadow-inner">
                                <x-lucide-user class="size-4 opacity-20" />
                                <input type="text" name="name" value="{{ old('name', $employee->user->name) }}" class="grow font-bold bg-transparent border-none focus:ring-0 text-sm" required />
                            </label>
                        ...
                        <div class="form-control group">
                            ...
                            <label class="input input-bordered flex items-center gap-4 bg-base-200/50 border-base-content/5 rounded-2xl h-14 focus-within:border-info transition-all shadow-inner">
                                ...
                                <input type="email" name="email" value="{{ old('email', $employee->user->email) }}" class="grow font-bold bg-transparent border-none focus:ring-0 text-sm" required />
                            </label>
                        ...
                        <div class="form-control group">
                            ...
                            <label class="input input-bordered flex items-center gap-4 bg-base-200/50 border-base-content/5 rounded-2xl h-14 focus-within:border-info transition-all shadow-inner">
                                ...
                                <select name="locale" class="grow font-bold bg-transparent border-none focus:ring-0 text-sm appearance-none cursor-pointer">
                                    <option value="id" {{ old('locale', $employee->user->locale) == 'id' ? 'selected' : '' }}>Bahasa Indonesia</option>
                                    <option value="en" {{ old('locale', $employee->user->locale) == 'en' ? 'selected' : '' }}>English (US)</option>
                                </select>
                            </label>
                        </div>
                    </div>
                ...
                <div class="bg-base-100 rounded-[2.5rem] p-10 border border-base-content/5 shadow-2xl relative overflow-hidden">
                    <div class="flex items-center gap-4 mb-10 pb-6 border-b border-base-content/5">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] font-bold opacity-30 uppercase tracking-[0.2em]">Work Info (NIP: {{ $employee->employee_code }})</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        <div class="form-control">
                            ...
                            <select name="gender" class="select select-bordered bg-base-200/50 rounded-2xl h-14 font-bold text-sm focus:border-secondary transition-all" required>
                                <option value="male" {{ old('gender', $employee->gender) == 'male' ? 'selected' : '' }}>{{ __('employee.gender.male') }}</option>
                                <option value="female" {{ old('gender', $employee->gender) == 'female' ? 'selected' : '' }}>{{ __('employee.gender.female') }}</option>
                            </select>
                        </div>
                    ...
                    <div class="form-control mt-6">
                        ...
                        <textarea name="address" class="textarea textarea-bordered bg-base-200/50 rounded-2xl min-h-[100px] font-bold text-sm w-full focus:border-secondary transition-all">{{ old('address', $employee->address) }}</textarea>
                    </div>

                    <div class="flex flex-col sm:flex-row gap-4 mt-12 pt-8 border-t border-base-content/5">
                        <button type="submit" class="btn btn-info flex-[2] h-16 rounded-2xl text-white shadow-xl shadow-info/20 hover:scale-[1.02] active:scale-95 transition-all font-black uppercase tracking-[0.2em]">
